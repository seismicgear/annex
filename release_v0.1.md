...
The federation protocol is architecturally complete:
...
- Policy-reactive trust re-evaluation on server policy changes
...
- Federation agreement lifecycle (establishment, modification, revocation)
...
The agent participation framework is implemented:
- Agent VRP handshake against server policy root
...
- Legacy Ledger reputation decay over time with accumulated handshake outcomes

---

## Known limitations

- **ZK proof generation requires trusted setup artifacts** — the `zk/keys/` directory must contain valid Groth16 keys. Without them, membership verification is unavailable and some integration tests are skipped.
...
- **Voice requires LiveKit** — voice channels are non-functional without a running LiveKit server and valid API credentials.

...
use annex_server::{middleware::{auth_middleware, IdentityContext}, AppState};
use axum::{
    body::Body,
...
fn load_vkey() -> Arc<annex_identity::zk::VerifyingKey<annex_identity::zk::Bn254>> {
    ...
    if !vkey_path.exists() {
        ...
        panic!("ZK keys not found at {:?}. Run `cargo test -p annex-identity` to generate them.", vkey_path);
    }
...
async fn test_auth_middleware_flow() {
    ...
    let app = Router::new()
        .route("/protected", get(|Extension(identity): Extension<IdentityContext>| async move {
